...
export function LoginClient() {
  ...
  const [setPasswordStatus, setSetPasswordStatus] = useState<string | null>(null);
      ...
      fetch("/api/auth/validate-invite", {
        ...
        .then((data) => {
          ...
          if (data.ok) {
            ...
            setSetPasswordUserName(data.name);
            setSetPasswordToken(token);
          } else {
            setSetPasswordStatus(data.error || "Invalid invitation link");
          }
        })
        .catch((err) => {
          if (!active) return;
          setSetPasswordStatus(err instanceof Error ? err.message : String(err));
        });
  ...
  async function login() {
    ...
    try {
      ...
      if (data.needsPassword) {
        setNeedsPassword(true);
        setSetPasswordEmail(data.email);
        setSetPasswordUserName(data.userName || "");
  ...
  async function setPasswordForUser() {
    ...
    try {
      const body = setPasswordToken 
        ? { token: setPasswordToken, password: newPassword, confirmPassword: confirmNewPassword }
        : { email: setPasswordEmail, password: newPassword, confirmPassword: confirmNewPassword };
    ...
    <div className="mx-auto flex min-h-[70vh] w-full max-w-xl flex-col justify-center space-y-6">
      ...
      {needsPassword ? (
        <section className="border border-amber-200 bg-amber-50 p-6 shadow-sm">
          <h2 className="text-sm font-semibold text-zinc-900">Set your password</h2>
          <p className="mt-1 text-xs text-zinc-600">
            Welcome{setPasswordUserName ? `, ${setPasswordUserName}` : ""}! Your account ({setPasswordEmail}) doesn&apos;t have a password yet. Please create one to continue.
          </p>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="grid gap-1.5">
              ...
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && setPasswordForUser()}
                className="form-field h-10"
                placeholder="••••••••"
                autoComplete="new-password"
              />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            >
              {isSettingPassword ? "Setting password…" : "Set password & Sign in"}
            </button>
            <button
              className="h-10 px-4 text-sm font-medium text-zinc-600 hover:text-zinc-900"
              onClick={cancelSetPassword}
              disabled={isSettingPassword}
            >
              Cancel
            </button>
            {setPasswordStatus ? <div className="text-xs text-red-600">{setPasswordStatus}</div> : null}
          </div>
        </section>
      ) : null}
