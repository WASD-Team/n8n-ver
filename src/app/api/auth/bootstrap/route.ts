...
import { ensureDefaultInstance, addMembership } from "@/lib/instancesStore";
...
export async function POST(request: Request) {
  try {
    ...
    const user = await createUserWithPassword({
      ...
      status: "Active",
      passwordHash,
      isSuperAdmin: true, // First user is always SuperAdmin
    });

    // Ensure default instance exists and add SuperAdmin to it
    const defaultInstance = await ensureDefaultInstance();
    await addMembership({
      userId: user.id,
    ...
    await logAudit({
      ...
      entityType: "user",
      entityId: user.id,
      details: { email: user.email, role: user.role, isSuperAdmin: true },
      instanceId: defaultInstance.id,
    });

    const response = NextResponse.json({ ok: true, user });
    response.cookies.set(AUTH_COOKIE, email, { httpOnly: true, sameSite: "lax", path: "/" });
    response.cookies.set(INSTANCE_COOKIE, defaultInstance.id, { httpOnly: true, sameSite: "lax", path: "/" });
    return response;
  } catch (err) {
    return NextResponse.json(
      { ok: false, error: err instanceof Error ? err.message : String(err) },
