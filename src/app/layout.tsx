...
import { getCurrentUser, getUserInstances, getCurrentInstanceId } from "@/lib/auth";
...
export default async function RootLayout({
  ...
  if (hasUserCookie) {
    // Load user and instances first - these are critical for navigation
    try {
      [currentUser, instances, currentInstanceId] = await Promise.all([
        getCurrentUser(),
        getUserInstances(),
    ...
    // Load workflows and folders separately - errors here shouldn't break navigation
    const effectiveInstanceId = currentInstanceId || instances[0]?.id || null;
    if (effectiveInstanceId) {
      try {
        [workflows, folders] = await Promise.all([
          listWorkflows({ instanceId: effectiveInstanceId }),
          listWorkflowGroups(effectiveInstanceId),
        ]);
      } catch {
        workflows = [];
        folders = {};
        ...
        <div className="min-h-dvh w-full">
          <div className="grid min-h-dvh grid-cols-1 md:grid-cols-[260px_1fr]">
            <aside className="flex flex-col border-b border-white/10 bg-[#0f1f3a] text-white md:border-b-0 md:border-r md:border-white/10">
              ...
              <SidebarNav workflows={workflows} initialFolders={folders} isSuperAdmin={currentUser?.isSuperAdmin ?? false} />
