...
import { getCurrentUser, getEffectiveInstanceId } from "@/lib/auth";
...
export default async function DashboardPage() {
  ...
  const [workflows, recentVersions, staleWorkflows, auditEvents, currentUser] = await Promise.all([
    listWorkflows({ instanceId: instanceId ?? undefined }),
    listRecentVersions(RECENT_LIMIT, instanceId ?? undefined),
    listStaleWorkflows(STALE_LIMIT, instanceId ?? undefined),
    listAudit({ limit: 200, instanceId: instanceId ?? undefined }),
    getCurrentUser(),
  ]);
  ...
  const restoredVersions = restoreIds.length > 0 ? await listVersionsByIds(restoreIds, instanceId ?? undefined) : [];
